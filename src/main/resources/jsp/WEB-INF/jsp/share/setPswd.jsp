...
<head>
    <meta charset="UTF-8">
    <title>${share.responseBody.shareFile.name } - ${share.responseBody.shareUser.username }的私密分享 - 爱易云盘</title>
...
<body>
    ...
    <div class="content">
		...
		<div class="right">
           <input id="pswdInput" placeholder="输入分享密码" name="pswd" /><button onClick="setPswd(${share.responseBody.shareFile.id });"> 进入分享 </button>
        </div>
...
<script type="text/javascript">
	function setPswd(fileId){
		var pswd = $("#pswdInput").val();
		$.post(fileId + "/_setPswd", {pswd:pswd}, function(response){
			if(response.result){
				location = location;
			}else{
